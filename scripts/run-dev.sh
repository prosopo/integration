...
docker exec -it $CONTAINER_NAME zsh -c '/usr/src/docker/dev.dockerfile.set.env.variables.sh && yarn && yarn build && cd /usr/src/provider && yarn setup provider && yarn setup dapp'
